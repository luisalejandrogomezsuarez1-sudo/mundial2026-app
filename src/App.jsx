...
function RadialBracket({ scores }) {
  ...
  // Pares agrupados: 16 partidos, cada uno ocupa 22.5°, los 2 rivales separados solo GAP°
  ...
  const elbow = (rO, d1, d2, rI, dMid, active1, active2) => {
    ...
    const anyA = active1 || active2;
    paths.push(<path key={`e${rO}-${d1}-1`} d={`M ${c1x} ${c1y} L ${b1x} ${b1y}`} fill="none" stroke={active1?GOLD:'#555'} strokeWidth={active1?1.3:0.7} opacity={active1?0.85:0.5}/>);
    paths.push(<path key={`e${rO}-${d1}-2`} d={`M ${c2x} ${c2y} L ${b2x} ${b2y}`} fill="none" stroke={active2?GOLD:'#555'} strokeWidth={active2?1.3:0.7} opacity={active2?0.85:0.5}/>);
    paths.push(<path key={`e${rO}-${d1}-arc`} d={arc(rBus, d1, d2)} fill="none" stroke="#555" strokeWidth={0.7} opacity={0.5}/>);
    paths.push(<path key={`e${rO}-${d1}-rad`} d={`M ${pmx} ${pmy} L ${nx} ${ny}`} fill="none" stroke={anyA?GOLD:'#555'} strokeWidth={anyA?1.3:0.7} opacity={anyA?0.85:0.5}/>);
  };
  ...
  for (let m=0;m<2;m++){ const [ax,ay]=P(R.sf, aSF[m]); paths.push(<path key={`sf${m}`} d={`M ${ax} ${ay} L ${cx} ${cy-72}`} fill="none" stroke={sfS[m]?.winner?GOLD:'#555'} strokeWidth={sfS[m]?.winner?1.3:0.7} opacity={sfS[m]?.winner?0.85:0.5}/>); }
  ...
  return (
    <svg viewBox="0 0 1000 1016" style={{ width:'100%', maxWidth:700, display:'block', margin:'0 auto' }}>
      ...
      {a32.map((a,p) => { const [x,y]=P(R.r32,a); const sl=r32S[p]||{};
        return <Node key={`n32${p}`} x={x} y={y} flag={sl.winnerFl} active={!!sl.winner} r={11} />; })}
      {a16.map((a,j) => { const [x,y]=P(R.r16,a); const sl=r16S[j]||{};
        return <Node key={`n16${j}`} x={x} y={y} flag={sl.winnerFl||sl.homeFl} active={!!sl.winner||!!sl.home} r={13} />; })}
      {aQF.map((a,m) => { const [x,y]=P(R.qf,a); const sl=qfS[m]||{};
        return <Node key={`nqf${m}`} x={x} y={y} flag={sl.winnerFl||sl.homeFl} active={!!sl.winner||!!sl.home} r={15} />; })}
      {aSF.map((a,s) => { const [x,y]=P(R.sf,a); const sl=sfS[s]||{};
        return <Node key={`nsf${s}`} x={x} y={y} flag={sl.winnerFl||sl.homeFl} active={!!sl.winner||!!sl.home} r={17} />; })}
